...
void MostrarCentimetros(uint16_t cm){
	...
	LCD_wr_char(c+48);
	LCD_wr_instruction(0b10000111);
	LCD_wr_char(d+48);
	LCD_wr_instruction(0b10001000);
	LCD_wr_char(u+48);
}

void MostrarLecutra(float res){
	uint16_t v = (uint16_t)res;
	MostrarCentimetros(res);
...
float ObtenerDistancia(){
	...
	/*Esperar sensor para estar listo*/
	//while(!(PORTA & (1<<1))){}
	while ((TIFR & (1 << ICF1)) == 0);
	...
	/*Calcular distancia dado el tiempo que tardó*/
	//uint16_t timepo = ICR1  + (veces*256);
	uint32_t tiempo = ICR1 + (65535 * veces);	/* Take count */
	TCCR0 = 0b00000000;
	float res = tiempo / 58.3;				//58 tiempo que tarda en 1 cm de ida y vuelta
	return res;
}
